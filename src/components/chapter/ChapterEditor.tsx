...
function Toolbar({ editor, readOnly }: ToolbarProps) {
  ...
  return (
    <div className="ed-toolbar inline-flex items-center gap-0.5 p-1 border-2 border-text bg-bg shadow-[0_3px_0_0_#000]">
      <ToolBtn
        title="撤销 (Ctrl+Z)"
...
export function ChapterEditor({
  ...
  return (
    <div className="flex flex-col">
      <div className="sticky top-0 z-10 bg-bg pb-3 mb-3 border-b border-border">
      ...
      {/* 字数统计底栏 */}
      <div className="mt-6 pt-3 border-t border-border flex flex-wrap items-center gap-4 text-xs text-2">
        <span>
          总字数 <span className="font-mono tnum text-text">{stats.total.toLocaleString()}</span>
        </span>
        <span>
          中文 <span className="font-mono tnum text-text">{stats.chinese.toLocaleString()}</span>
        </span>
        <span>
          英文 <span className="font-mono tnum text-text">{stats.english.toLocaleString()}</span>
        </span>
        <span>
          段落 <span className="font-mono tnum text-text">{stats.paragraphs}</span>
        </span>
        <span>
          阅读 <span className="font-mono tnum text-text">~{stats.readMinutes} 分钟</span>
        </span>
        {readOnly && (
          <span className="ml-auto inline-flex items-center gap-1 text-3">
