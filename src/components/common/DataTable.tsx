...
export function DataTable<T>({
  ...
  return (
    <div className={cn('bd', className)}>
      {/* header */}
      <div
        className={cn(
          'grid items-center gap-4 px-4 py-2 text-xs font-medium uppercase tracking-wider text-2 border-b border-text bg-bg',
      ...
      {items.map((item, idx) => {
          ...
          <>
            {rowIcon && (
              <div className="flex items-center justify-center">
                {Icon ? <Icon className="w-4 h-4" strokeWidth={1.5} /> : null}
              </div>
            )}
            {columns.map((c) => (
              <div key={c.key} className={c.className}>
        ...
        return (
          <div
            ...
            className={cn(
              'grid items-center gap-4 px-4 py-3 border-b border-border last:border-b-0 transition-colors',
