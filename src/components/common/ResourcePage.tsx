...
export function ResourcePage<TFilter extends string = string>({
  ...
  return (
    <div className={cn('w-full', maxWidthClassName ?? 'max-w-7xl')}>
      <PageHeader
        eyebrow={eyebrow}
        title={title}
        subtitle={subtitle}
      ...
      {stats && stats.length > 0 && <StatStrip items={stats} className="mb-6" />}
        ...
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          ...
          {search && (
            <div className="relative w-56 max-w-full">
              <Search
                className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-3 pointer-events-none"
                strokeWidth={1.5}
              />
              <Input
                value={search.value}
