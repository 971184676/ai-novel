    ...
    <div className="space-y-6 sm:space-y-8 lg:space-y-10">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1 sm:gap-2 w-full sm:w-auto flex-wrap">
          ...
          <Link to={`/book/${bookId}/chapters`} className="w-full sm:w-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        <section className="col-span-1 lg:col-span-8 space-y-6 lg:space-y-10">
          <ModuleGrid bookId={bookId} progress={progress} />
          <RecentChapters bookId={bookId} chapters={recentChapters ?? []} />
        </section>
        <aside className="col-span-1 lg:col-span-4 space-y-6 lg:space-y-8">
          <StatsPanel
...
function BookHeader({ book, onEdit }: { book: Book; onEdit: () => void }) {
  return (
    <header className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 sm:gap-6">
      <div className="min-w-0 w-full sm:w-auto">
        <div className="text-xs tnum text-3 mb-1.5 sm:mb-2">OVERVIEW · 书籍总览</div>
        <h1 className="text-lg sm:text-xl lg:text-2xl font-semibold leading-tight">{book.name}</h1>
        {book.description && (
          <p className="text-xs sm:text-sm text-2 mt-1.5 sm:mt-2 max-w-sm sm:max-w-xl leading-relaxed">{book.description}</p>
        )}
        <div className="flex items-center gap-1.5 sm:gap-2 mt-2 sm:mt-3 text-xs text-2 tnum flex-wrap">
          <span className="border-2 border-text px-1.5 sm:px-2 py-0.5 bg-bg text-text font-medium text-xs">{book.genre}</span>
          <span
            className={cn(
              'inline-flex items-center h-5 px-1.5 sm:px-2 border-2 font-semibold text-xs',
          ...
          <span className="text-3 hidden sm:inline">
...
function LevelTrack({
  ...
  return (
    <section>
      ...
      <div className="border-2 border-text grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 shadow-[0_5px_0_0_#000]">
        {levels.map((l) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'border-r-2 border-text last:border-r-0 px-2 sm:px-3 py-3 sm:py-4 bg-bg text-text',
      ...
      {current && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-2 sm:mt-3 text-xs text-2">
          <span>
            下一个：{current.name} · <span className="text-3 text-xs">{current.description}</span>
          </span>
          <span className="tnum mt-1 sm:mt-0">
        ...
        {MODULES.map((m) => {
          ...
          return (
            <Link
              ...
              to={m.href(bookId)}
              className={cn(
                'card-3d p-3 sm:p-4 flex flex-col gap-1.5 sm:gap-2',
                locked && 'opacity-50',
              )}
            >
              <div className="flex items-center justify-between">
                <m.icon className="w-3.5 sm:w-4 h-3.5 sm:h-4" strokeWidth={1.5} />
                {done ? (
                  <CheckCircle2 className="w-2.5 sm:w-3 h-2.5 sm:h-3" strokeWidth={1.5} />
                ) : locked ? (
                  <Lock className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-3" strokeWidth={1.5} />
                ) : (
              ...
              <div className="text-xl sm:text-2xl font-semibold tnum leading-none">{n}</div>
