...
function TopBar() {
  return (
    <header className="border-b border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-3 sm:py-4 lg:py-6 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1 sm:gap-2">
          <Link to="/settings">
            <Button size="icon" className="lg:hidden" title="设置">
...
function Hero({ stats }: { stats: { total: number; ongoing: number; completed: number; totalChapters: number } }) {
  return (
    <section className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8 mb-8 md:mb-12">
      <div className="col-span-1 md:col-span-7">
        <div className="text-xs tnum text-3 mb-2 md:mb-3">SECTION 01 / LIBRARY</div>
        <h1 className="text-xl md:text-2xl lg:text-3xl font-semibold mb-2 md:mb-3 leading-tight">书籍库</h1>
        <p className="text-xs md:text-sm text-2 max-w-md leading-relaxed">
          所有作品保存在本地浏览器。从创建一本新书开始，逐步构建世界观、人物、地图，最终输出一部完整小说。
        </p>
      </div>
      <div className="col-span-1 md:col-span-5">
        <div className="panel-3d">
          <div className="flex items-baseline justify-between p-3 md:p-4 border-b-2 border-text">
            <div className="text-xs tnum text-3">TOTAL · 累计</div>
            <div className="text-xl md:text-2xl font-semibold tnum">{stats.total}</div>
          </div>
...
function EmptyState() {
  return (
    <div className="panel-3d py-12 px-4 sm:py-20 sm:px-8 text-center">
      <div className="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 border-2 border-text mb-4 sm:mb-6 shadow-[0_4px_0_0_#000]">
        <BookOpen className="w-6 h-6 sm:w-8 sm:h-8" strokeWidth={1.5} />
      </div>
      <h2 className="text-lg sm:text-xl font-semibold mb-2 sm:mb-3">创建你的第一部小说</h2>
      <p className="text-xs sm:text-sm text-2 leading-relaxed max-w-sm sm:max-w-md mx-auto mb-6 sm:mb-8">
      ...
      <div className="mt-8 sm:mt-12 grid grid-cols-2 sm:grid-cols-4 gap-2 max-w-lg sm:max-w-2xl mx-auto">
        {LEVELS.map((l) => (
          <div key={l.level} className="border-2 border-text bg-bg p-3 sm:p-4 text-left shadow-[0_3px_0_0_#000]">
