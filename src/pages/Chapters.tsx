    ...
    <div className="flex flex-col gap-4 h-[calc(100vh-120px)] min-h-[600px]">
      ...
      <div className="flex items-baseline justify-between">
        <div>
          ...
          <h1 className="text-2xl font-semibold mt-1">章节创作</h1>
      ...
      {/* 三栏 */}
      <div className="flex-1 min-h-0 grid grid-cols-[260px_1fr] border-2 border-text bg-bg shadow-[0_5px_0_0_#000]">
        <ChapterList
          chapters={chapters ?? []}
  ...
  return (
    <aside className="border-r border-border flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-y-auto">
        {chapters.length === 0 ? (
          <div className="px-4 py-12 text-center text-xs text-3">
            还没有章节
            <br />
            点击右上「新建章节」开始
          </div>
      ...
      >
        <div className="border-b border-border px-6">
          <TabsList>
            ...
            <TabsTrigger value="outline">设置</TabsTrigger>
          </TabsList>
        </div>

        <TabsContent value="content" className="flex-1 min-h-0 overflow-auto mt-0 px-12 py-8">
          <ContentPane chapter={chapter} />
        </TabsContent>

        <TabsContent value="outline" className="flex-1 min-h-0 overflow-auto mt-0 px-12 py-8">
  ...
  return (
    <div className="px-6 py-3 border-b border-border flex items-center justify-between gap-4">
      <div className="min-w-0">
        <div className="flex items-center gap-2 text-xs text-2 tnum">
        ...
        <h2 className="text-md font-semibold mt-0.5 truncate">
          {chapter.title || '未命名章节'}
        </h2>
      </div>
      <div className="flex items-center gap-2 text-xs text-3 tnum shrink-0">
...
function StateMachineBar({ chapter }: { chapter: Chapter }) {
  ...
  return (
    <>
      <div className="px-6 py-3 bg-surface border-b border-border flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs tnum text-2 mr-1">创作流程</span>
          {STATUS_FLOW.map((s, i) => (
            <span key={s} className="flex items-center gap-2">
              <StatusBadge status={s} />
              {i < STATUS_FLOW.length - 1 && (
                <ArrowRight className="w-3 h-3 text-3" strokeWidth={1.5} />
              )}
            </span>
          ))}
        </div>
      ...
      {/* final 状态：底部只读提示条 */}
      {isFinal && !unlocked && (
        <div className="px-6 py-1.5 bg-bg border-b border-border text-xs text-2">
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-border">
        <div className="text-xs tnum text-2">
          {stats.total.toLocaleString()} 字 · 中文 {stats.chinese} / 英文 {stats.english} · 阅读 ~{stats.readMinutes} 分钟
        </div>
        <div className="flex items-center gap-2">
          {!hasApiKey() && (
            <Link to="/settings" className="text-xs text-2 underline hover:text-text inline-flex items-center gap-1">
              <Settings className="w-3 h-3" strokeWidth={1.5} /> 配置 API Key
            </Link>
          )}
          {streaming ? (
