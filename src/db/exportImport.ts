...
export async function exportAll(): Promise<{
  ...
  const json = JSON.stringify(bundle, null, 2);
  const size = new Blob([json]).size;
  const filename = `novel-creator-full-backup-${formatStamp()}.json`;
...
export function validateBackup(rawText: string): ValidationResult {
  ...
  if (obj.format !== 'novel-creator-backup') {
    return { ok: false, error: '文件格式标识不匹配（期望 "novel-creator-backup"）' };
  }
